# Automated Merge Script for BearMod Project
# Usage: .\merge-updates.ps1 [source_path]
...
function Merge-CppFiles {
    ...
    # Special handling for CMakeLists.txt
    $sourceCMake = "$sourceCppPath\CMakeLists.txt"
    $destCMake = "$destCppPath\CMakeLists.txt"
    
    if (Test-Path $sourceCMake) {
        if (Test-Path $destCMake) {
            Copy-Item -Path $destCMake -Destination "$destCMake.bak" -Force
        }
        
        try {
            # Instead of direct copy, we'll create a fixed version
            $cmakeContent = Get-Content -Path $sourceCMake -Raw
            
            # Fix common issues in CMakeLists.txt
            $cmakeContent = $cmakeContent -replace "c\+\+_static", "c++_shared"
            
            # Add exception and RTTI flags if not present
            if ($cmakeContent -notmatch "-fexceptions") {
                $cmakeContent = $cmakeContent -replace "set\(CMAKE_CXX_FLAGS (.*?)\)", 'set(CMAKE_CXX_FLAGS $1 -fexceptions -frtti)'
            }
            
            Set-Content -Path $destCMake -Value $cmakeContent
            Write-Log "Merged and fixed: CMakeLists.txt"
        }
        catch {
            Write-Log "Error merging CMakeLists.txt: $_"
        }
    }
